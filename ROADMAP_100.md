...
- [x] **63.** `DEPLOY.md` rewritten — two topologies (Vercel free, Docker ministry), full env-var table with `TRUSTED_PROXY_IPS` / `TURNSTILE_SECRET` / `JWT_SECRET_PREVIOUS`, eight-workflow CI checklist, what-changed-in-this-revision section.
- [x] **64.** `DOCKER_MINISTRY_DEPLOY.md` rewritten — hardware baseline table, three deploy tiers, full env-var table including `TRUSTED_PROXY_IPS`, sanity-check curls, air-gapped notes, cost rough-cut.
- [x] **65.** `DEMO_ACCOUNTS.md` rewritten — 5 demo users, password `demo1234`, seeding instructions, reset SQL, M1 testing notes.
